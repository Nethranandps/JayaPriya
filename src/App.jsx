...
import { motion, MotionConfig, useInView, useReducedMotion, useScroll, useSpring, useTransform } from 'framer-motion';
...
function Counter({ value, suffix = '', prefix = '', duration = 2, delay = 0 }) {
  ...
  useEffect(() => {
    ...
    const animate = (now) => {
      ...
      setAnimatedValue(Math.floor(endValue * eased));
      if (progress < 1) frame = requestAnimationFrame(animate);
    };
    const timer = setTimeout(() => { frame = requestAnimationFrame(animate); }, delay);
    return () => { clearTimeout(timer); cancelAnimationFrame(frame); };
  }, [inView, value, duration, delay]);

  return <span ref={ref} className="counter-value">{prefix}{animatedValue.toLocaleString()}{suffix}</span>;
}
...
function ProjectVisual({ project }) {
  return <div className={`project-visual visual-${project.theme}`} aria-hidden="true">
    ...
    {project.id === 'paid' && <div className="paid-visual"><div className="paid-rings"><i /><i /><i /><Target size={36} /></div><div className="paid-label meta">High-intent audiences</div><div className="paid-bars">{[20, 31, 25, 45, 38, 55, 48, 70, 62, 86, 79, 100].map((n, i) => <i key={i} style={{ height: `${n}%` }} />)}</div></div>}
...
function ContactParticleCanvas() {
  ...
  useEffect(() => {
    ...
    function setup() {
      ...
      // Particles the cursor hits take on the cursor's own speed (1.0 = physically accurate, as in the
      // Ten Minute Physics FLIP reference and the FLIP Fluids 'velocity scale'); higher values exaggerate.
      fluid.obstacleKick = 1;
    ...
    function tick(time) {
      ...
      // Cursor obstacle: grows with cursor speed, like the reference. The velocity is smoothed over a few
      // frames (pointer events arrive unevenly, so raw per-frame deltas spike) and capped at a fast hand
      // sweep, so a flick nudges the pile instead of launching it across the section.
      const maxSpeed = width * 0.6;
      if (pointer.active) {
        const rawVx = (pointer.x - pointer.px) / dt;
        const rawVy = (pointer.y - pointer.py) / dt;
        pointer.vx += (rawVx - pointer.vx) * 0.35;
        pointer.vy += (rawVy - pointer.vy) * 0.35;
        const rawSpeed = Math.hypot(pointer.vx, pointer.vy);
        if (rawSpeed > maxSpeed) { pointer.vx *= maxSpeed / rawSpeed; pointer.vy *= maxSpeed / rawSpeed; }
      } else { pointer.vx = 0; pointer.vy = 0; }
      const speed = Math.hypot(pointer.vx, pointer.vy);
      let factor = pointer.touch ? (pointer.holding ? 0.35 : 0) : fit(speed / maxSpeed, 0, 1, 0.2, 1);
      if (pouring) factor = 0;
      // Reference obstacle size: about 5% of the domain height at full speed.
      const maxRadius = Math.min(70, height * 0.05);
      const obstacle = pointer.active && factor > 0
        ? { x: pointer.x, y: pointer.y, radius: maxRadius * factor, vx: pointer.vx, vy: pointer.vy }
    ...
    <section className="contact-section section dark-section relative" id="contact">
      ...
      <div className="container relative" style={{ zIndex: 1 }}>
        ...
        <motion.footer initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.5 }}>
          <a className="wordmark" href="#top" data-magnetic>JAYA PRIYA</a>
    ...
    <main id="main">
      <section className="hero-section container relative" id="top">
        ...
        <motion.div className="hero-meta meta" initial={{ opacity: 0, y: -14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <span>Independent thinking. Measurable impact.</span>
        </motion.div>
        ...
        <div className="container relative z-10">
          ...
          <ScrollReveal delay={0.15} className="automation-grid">
            ...
            <div className="workflow-demo">
              <div className="meta workflow-title"><span>A lead's journey</span><span>Interactive walkthrough</span></div>
      ...
      <ScrollReveal className="section stack-section" id="stack" delay={0.1}>
        <div className="container">
          ...
          <ScrollReveal delay={0.15} className="stack-grid">
            {[{ title: 'Search & analytics', items: ['Ahrefs', 'Semrush', 'Google Analytics', 'Search Console'], icon: Search }, { title: 'Paid & performance', items: ['Google Ads', 'Meta Ads', 'Google Tag Manager'], icon: Target }, { title: 'Content & creative', items: ['WordPress', 'Canva', 'Blog', 'Social media content'], icon: FileText }, { title: 'Outreach & automation', items: ['LinkedIn', 'Email', 'Apollo.io', 'HubSpot', 'Clay', 'HeyReach', 'n8n', 'Make.com'], icon: Workflow }].map(({ title, items, icon: Icon }, i) => (
              <motion.div
                key={title}
...
<ScrollReveal className="section experience-section" id="experience" delay={0.1}>
        <div className="container">
          <SectionHeading label="06 / The experience" aside="Built by doing">
            THINKING BIG<br />WORKING HANDS-ON
          </SectionHeading>
          <ScrollReveal delay={0.15} className="experience-list">
            {[{ company: 'Coderead', role: 'Performance Marketing Specialist', category: 'Performance', result: '800+ qualified leads', description: 'Managed Meta Ads campaigns, with continuous attention to cost per lead, creative performance, and conversion rates.', project: 1 }, { company: 'Cogent Innovation', role: 'LinkedIn & Email Marketing Specialist', category: 'B2B & outreach', result: '80+ organic B2B leads', description: 'Built organic LinkedIn outreach and personalized email sequences to connect with relevant business decision-makers.', project: 2 }, { company: 'FirstHub Ecom Pvt Ltd', role: 'Digital Marketing Executive', category: 'Full-funnel marketing', result: '10× online sales growth', description: 'Owned SEO, paid ads, and analytics end-to-end — led product-page optimization and site health improvements that scaled online revenue.', project: 0 }, { company: 'EMC', role: 'SEO & Content Executive', category: 'SEO & content', result: '30+ videos SEO optimized', description: 'Optimized on-page SEO and 30+ YouTube videos for search visibility, while managing social account across platforms.', project: null }].map((exp, i) => (
              <motion.details
                key={exp.company}
              ...
              >
                ...
                <div className="experience-detail">
                  <p>{exp.description}</p>
                  {exp.project != null && <motion.button
      ...
      <ScrollReveal className="resume-section" id="resume" delay={0.1}>
        <div className="container">
          <div className="resume-header">
            <span className="meta accent">07 / The complete picture</span>
